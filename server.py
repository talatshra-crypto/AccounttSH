...
class QuotesDAO(BaseDAO):
    @staticmethod
    def list(c):
        rows_q = BaseDAO.rows(c.execute(
            "SELECT q.*, c.name AS linked_customer_name, c.phone AS linked_customer_phone "
            "FROM quotes q LEFT JOIN customers c ON c.id=q.customer_id ORDER BY q.id DESC"))
        for r in rows_q:
            r["items"] = QuotesDAO.get_items(c, r["id"])
        return rows_q
    @staticmethod
    def get_by_id(c, qid):
        r = BaseDAO.row1(c.execute(
            "SELECT q.*, c.name AS linked_customer_name, c.phone AS linked_customer_phone "
            "FROM quotes q LEFT JOIN customers c ON c.id=q.customer_id WHERE q.id=?", (qid,)))
        if r: r["items"] = QuotesDAO.get_items(c, qid)
        return r
    @staticmethod
    def get_items(c, qid):
        return BaseDAO.rows(c.execute(
            "SELECT qi.*, p.name AS product_name, p.barcode, p.unit "
            "FROM quote_items qi LEFT JOIN products p ON p.id=qi.product_id "
            "WHERE qi.quote_id=?", (qid,)))
    @staticmethod
    def create(c, data, total):
        cur = c.execute(
            "INSERT INTO quotes(customer_id,customer_name,customer_phone,date,valid_until,status,total,notes) "
            "VALUES(?,?,?,?,?,?,?,?)",
            (data.get("customer_id"), data.get("customer_name",""), data.get("customer_phone",""),
             data.get("date",""), data.get("valid_until",""), data.get("status","مسودة"), total, data.get("notes","")))
        return cur.lastrowid
    @staticmethod
    def update(c, qid, data, total):
        c.execute(
            "UPDATE quotes SET customer_id=?,customer_name=?,customer_phone=?,date=?,valid_until=?,status=?,total=?,notes=? WHERE id=?",
            (data.get("customer_id"), data.get("customer_name",""), data.get("customer_phone",""),
             data.get("date",""), data.get("valid_until",""), data.get("status","مسودة"), total, data.get("notes",""), qid))
    @staticmethod
    def update_status(c, qid, status):
        c.execute("UPDATE quotes SET status=? WHERE id=?", (status, qid))
    @staticmethod
    def set_converted(c, qid, sale_id, service_id):
        c.execute("UPDATE quotes SET status='مقبول', converted_sale_id=?, converted_service_id=? WHERE id=?",
                  (sale_id, service_id, qid))
    @staticmethod
    def delete(c, qid):
        c.execute("DELETE FROM quotes WHERE id=?", (qid,))
    @staticmethod
    def create_item(c, qid, kind, product_id, description, qty, price):
        c.execute("INSERT INTO quote_items(quote_id,kind,product_id,description,qty,price) VALUES(?,?,?,?,?,?)",
                  (qid, kind, product_id, description, qty, price))
    @staticmethod
    def delete_items(c, qid):
...
class ReportsDAO(BaseDAO):
    ...
    def supplier_report(c, sup_id, date_from, date_to):
        ...
        opening_balance = sup.get("balance") or 0
    ...
    def customer_report(c, cust_id, date_from, date_to):
        ...
        opening_balance = cust.get("balance") or 0
        total_paid_final = total_paid + account_paid
        total_remain = max(0, total_remain_inv + opening_balance - account_paid)
    ...
    def payments_summary(c, party_type, party_id):
        ...
        if party_type == "supplier":
            party_row = SuppliersDAO.get_by_id(c, party_id)
            ...
            party_row = CustomersDAO.get_by_id(c, party_id)
            invs = BaseDAO.rows(c.execute(
        ...
        opening_balance = (party_row.get("balance") or 0) if party_row else 0
        ...
        return {"invoices": result, "total_invoices": total_inv,
                ...
                "account_paid": account_paid, "opening_balance": opening_balance,
                "total_remaining": max(0, total_inv + opening_balance - total_paid)}
...
def handle_api(method, path, body, req):
    ...
    if ep == "quotes":
        if method == "GET" and len(parts) == 2:
            r = QuotesDAO.list(c); c.close(); return 200, r
        if method == "GET" and len(parts) == 3:
            r = QuotesDAO.get_by_id(c, int(parts[2]))
            if not r: c.close(); return 404, {"detail":"عرض السعر غير موجود"}
            c.close(); return 200, r
        if method == "POST" and len(parts) == 2:
            items = body.get("items", [])
            if not items:
                c.close(); return 400, {"detail":"يجب إضافة صنف واحد على الأقل"}
            if not body.get("customer_id") and not body.get("customer_name","").strip():
                c.close(); return 400, {"detail":"يجب اختيار زبون مسجّل أو كتابة اسم"}
            if not body.get("date"):
                c.close(); return 400, {"detail":"يجب تحديد التاريخ"}
            total = sum(i["qty"]*i["price"] for i in items)
            qid = QuotesDAO.create(c, body, total)
            for i in items:
                QuotesDAO.create_item(c, qid, i.get("kind","product"), i.get("product_id"),
                                      i.get("description",""), i["qty"], i["price"])
            c.commit()
            r = QuotesDAO.get_by_id(c, qid); c.close(); return 201, r
        if method == "PUT" and len(parts) == 3:
            qid = int(parts[2])
            if not QuotesDAO.get_by_id(c, qid):
                c.close(); return 404, {"detail":"عرض السعر غير موجود"}
            items = body.get("items", [])
            total = sum(i["qty"]*i["price"] for i in items)
            QuotesDAO.delete_items(c, qid)
            for i in items:
                QuotesDAO.create_item(c, qid, i.get("kind","product"), i.get("product_id"),
                                      i.get("description",""), i["qty"], i["price"])
            QuotesDAO.update(c, qid, body, total)
            c.commit()
            r = QuotesDAO.get_by_id(c, qid); c.close(); return 200, r
        if method == "DELETE" and len(parts) == 3:
            QuotesDAO.delete(c, int(parts[2]))
            c.commit(); c.close(); return 200, {"ok": True}
        if method == "POST" and len(parts) == 4 and parts[3] == "status":
            qid = int(parts[2])
            new_status = body.get("status","")
            if new_status not in ("مسودة","مرسل","مقبول","مرفوض"):
                c.close(); return 400, {"detail":"حالة غير صالحة"}
            QuotesDAO.update_status(c, qid, new_status)
            c.commit()
            r = QuotesDAO.get_by_id(c, qid); c.close(); return 200, r
        if method == "POST" and len(parts) == 4 and parts[3] == "convert":
            qid = int(parts[2])
            q = QuotesDAO.get_by_id(c, qid)
            if not q: c.close(); return 404, {"detail":"عرض السعر غير موجود"}
            if q.get("converted_sale_id") or q.get("converted_service_id"):
                c.close(); return 400, {"detail":"تم تحويل هذا العرض مسبقاً"}
            customer_id = q.get("customer_id")
            if not customer_id:
                name = q.get("customer_name","").strip()
                if not name:
                    c.close(); return 400, {"detail":"لا يوجد اسم زبون لإنشاء حساب له"}
                customer_id = CustomersDAO.create(c, {"name": name, "phone": q.get("customer_phone","")})
            ...
            if product_items:
                sale_total = sum(i["qty"]*i["price"] for i in product_items)
            ...
            if service_items:
                ...
                service_id = ServiceOrdersDAO.create(c, {
                    "customer_id": customer_id, "service_type": "تركيب", "device_desc": device_desc,
                    "issue_desc": f"محوّل من عرض سعر #{qid}", "status": "قيد الانتظار",
                    "received_date": today, "service_fee": svc_fee, "warranty_days": 0,
                    "notes": q.get("notes",""),
                })
            QuotesDAO.set_converted(c, qid, sale_id, service_id)
            c.commit()
            r = QuotesDAO.get_by_id(c, qid)
            c.close(); return 200, {"quote": r, "sale_id": sale_id, "service_id": service_id}
...
// إجمالي المستحق (غير المسدد) لمورد معيّن من جميع فواتير الشراء غير المردودة، مطروحاً منه دفعات الحساب المباشرة، مضافاً إليه الرصيد الافتتاحي
function supplierPayable(supId){
  const sup = suppliers.find(s=>s.id===parseInt(supId));
  const openingBalance = parseFloat(sup?.balance||0) || 0;
  ...
  return Math.max(0, invoicesDue + openingBalance - supplierAccountPaid(supId));
}

// إجمالي المستحق على زبون معيّن من كل مبيعاته وطلبات خدماته، مطروحاً منه دفعات الحساب المباشرة، مضافاً إليه الرصيد الافتتاحي
function customerReceivable(custId){
  const cust = customers.find(c=>c.id===parseInt(custId));
  const openingBalance = parseFloat(cust?.balance||0) || 0;
  ...
  return Math.max(0, invoicesDue + openingBalance - customerAccountPaid(custId));
}
...
async function loadAll(){
  ...
  try{
    const [p,s,cu,pu,sl,st,pays,cfg,expCats,exps,emps,payr,recExp,svcOrd,acctPays,qts] = await Promise.all([
      ...
      api('GET','/api/service_orders'), api('GET','/api/payments?ref_type=account'), api('GET','/api/quotes')
    ]);
    ...
    serviceOrders=svcOrd; accountPayments=acctPays; quotes=qts;
...
const NAV=[
  ...
  {id:'quotes',label:'عروض الأسعار',ic:'M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8zM14 2v6h6M9 15h6M9 11h2'},
...
function pageHTML(){
  ...
  if(page==='quotes')     return quotesHTML();
...
function entityHTML(type,title,sub){
  ...
  <div class="card"><table>
    <thead><tr><th>الاسم</th><th>الهاتف</th><th>المدينة</th><th>المستحق الفعلي</th><th></th></tr></thead>
    <tbody id="etb"></tbody>
  </table></div>`;
}
function renderEntity(type,q=''){
  ...
  tb.innerHTML=f.map(x=>{
    ...
    const actualDue = type==='suppliers' ? supplierPayable(x.id) : customerReceivable(x.id);
    const openingBalance = parseFloat(x.balance||0) || 0;
    return `<tr>
    ...
    <td><span class="badge ${actualDue!==0?'y':'g'}">${actualDue.toLocaleString()} ${cur()}</span>${openingBalance?`<div style="font-size:10px;color:#64748b;margin-top:2px;">منها ${openingBalance.toLocaleString()} ${cur()} رصيد افتتاحي</div>`:''}</td>
    <td><div style="display:flex;gap:5px;">
...
// ACCOUNTING
function accSumContentHTML(){
  const tsSales    = sales.reduce((s,x)=>s+x.total, 0);
  const tsServices = serviceOrders.reduce((s,x)=>s+(x.service_fee||0), 0);
  const ts = tsSales + tsServices;
  ...
  <div style="display:grid;grid-template-columns:repeat(6,1fr);gap:14px;margin-bottom:22px;">
    <div class="stat">
      <div style="font-size:12px;color:#64748b;">المبيعات</div>
      <div style="font-size:18px;font-weight:800;color:#52b788;margin-top:6px;">${tsSales.toLocaleString()} ${cur()}</div>
    </div>
    <div class="stat">
      <div style="font-size:12px;color:#64748b;">إيراد الخدمات</div>
      <div style="font-size:18px;font-weight:800;color:#a78bfa;margin-top:6px;">${tsServices.toLocaleString()} ${cur()}</div>
    </div>
    <div class="stat">
      <div style="font-size:12px;color:#64748b;">إجمالي المبيعات والخدمات</div>
      <div style="font-size:18px;font-weight:800;color:#34d399;margin-top:6px;">${ts.toLocaleString()} ${cur()}</div>
    </div>
    <div class="stat">
...
function accountingHTML(){
  ...
  <div id="acc-content">${accSumContentHTML()}</div>`;
}
...
//  صفحة عروض الأسعار
// ══════════════════════════════════════════════
const QUOTE_STATUSES = ['مسودة','مرسل','مقبول','مرفوض'];
const QUOTE_STATUS_CLS = {'مسودة':'b','مرسل':'y','مقبول':'g','مرفوض':'r'};

function quoteDaysLeft(q){
  if(!q.valid_until) return null;
  return Math.ceil((new Date(q.valid_until) - new Date()) / 86400000);
}
// عروض قاربت تنتهي (خلال 3 أيام) أو انتهت فعلاً ولسا معلّقة (مسودة/مرسل)
function getExpiringQuotes(){
  return quotes.filter(q=>{
    if(!['مسودة','مرسل'].includes(q.status)) return false;
    const d = quoteDaysLeft(q);
    return d !== null && d <= 3;
  });
}
...
function quotesHTML(){
  ...
  const accepted = quotes.filter(q=>q.status==='مقبول').length;
  return `<div class="ti">عروض الأسعار</div><div class="sub">إنشاء ومتابعة عروض الأسعار للزبائن</div>
  <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:18px;">
    <div class="stat"><div style="font-size:12px;color:#64748b;">إجمالي العروض</div><div style="font-size:18px;font-weight:800;color:#60a5fa;margin-top:6px;">${quotes.length}</div></div>
    <div class="stat"><div style="font-size:12px;color:#64748b;">مقبولة</div><div style="font-size:18px;font-weight:800;color:#52b788;margin-top:6px;">${accepted}</div></div>
    <div class="stat"><div style="font-size:12px;color:#64748b;">قيمة كل العروض</div><div style="font-size:18px;font-weight:800;color:#fbbf24;margin-top:6px;">${totalVal.toLocaleString()} ${cur()}</div></div>
    <div class="stat"><div style="font-size:12px;color:#64748b;">قاربت تنتهي</div><div style="font-size:18px;font-weight:800;color:#f87171;margin-top:6px;">${getExpiringQuotes().length}</div></div>
  </div>
  <div style="display:flex;gap:10px;margin-bottom:14px;flex-wrap:wrap;align-items:center;">
    <button class="btn p" id="qt-add-btn">+ عرض سعر جديد</button>
    <select class="inp" id="qt-status-filter" style="width:180px;">
      <option value="">كل الحالات</option>
      ${QUOTE_STATUSES.map(s=>`<option value="${s}">${s}</option>`).join('')}
    </select>
  </div>
  ${qtExpiryAlertsHTML()}
  <div id="qt-list">${qtListHTML(quotes)}</div>`;
}

function qtExpiryAlertsHTML(){
  const list = getExpiringQuotes();
  if(!list.length) return '';
  return `<div class="card" style="padding:16px;margin-bottom:16px;border:1px solid #5c4a23;">
    <div style="font-weight:700;color:#fbbf24;margin-bottom:12px;">⏰ عروض أسعار قاربت تنتهي (${list.length})</div>
    ${list.map(q=>{
      const d = quoteDaysLeft(q);
      const nm = q.customer_id ? esc(q.linked_customer_name) : esc(q.customer_name);
      return `<div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid #1e2537;">
      <span style="font-size:13px;color:#f1f5f9;font-weight:700;">#${q.id} — ${nm||'—'}</span>
      <span class="badge ${d<0?'r':'y'}">${d<0?`⏰ انتهى منذ ${Math.abs(d)} يوم`:d===0?'⚠️ ينتهي اليوم':`⚠️ ينتهي خلال ${d} يوم`}</span>
    </div>`;}).join('')}
  </div>`;
}

function qtListHTML(list){
  return `<div class="card" style="overflow:hidden;">
    <table>
      <thead><tr><th>#</th><th>الزبون</th><th>التاريخ</th><th>صالح حتى</th><th>الحالة</th><th>الإجمالي</th><th>إجراءات</th></tr></thead>
      <tbody>
      ${list.map(q=>{
        const nm = q.customer_id ? (q.linked_customer_name||'—') : (q.customer_name||'—');
        const d = quoteDaysLeft(q);
        const expired = d!==null && d<0 && ['مسودة','مرسل'].includes(q.status);
        return `<tr>
        <td style="color:#64748b;">#${q.id}</td>
        <td style="font-weight:700;color:#f1f5f9;">${esc(nm)}${!q.customer_id?' <span class="badge y" style="font-size:10px;">غير مسجّل</span>':''}</td>
        <td style="color:#94a3b8;">${q.date}</td>
        <td style="color:${expired?'#f87171':'#94a3b8'};">${q.valid_until||'—'}${expired?' ⏰':''}</td>
        <td><span class="badge ${QUOTE_STATUS_CLS[q.status]||'b'}">${q.status}</span></td>
        <td style="font-weight:700;">${(q.total||0).toLocaleString()} ${cur()}</td>
        <td><div style="display:flex;gap:4px;flex-wrap:wrap;">
          <button class="btn s" style="padding:4px 8px;font-size:11px;" onclick="openQuoteDetail(${q.id})">📋</button>
          ${!q.converted_sale_id&&!q.converted_service_id?`<button class="btn s" style="padding:4px 8px;font-size:11px;" onclick="openQuoteForm(${q.id})">✏️</button>`:''}
          ${!q.converted_sale_id&&!q.converted_service_id?`<button class="btn d" style="padding:4px 8px;font-size:11px;" onclick="deleteQuote(${q.id})">🗑️</button>`:''}
        </div></td>
      </tr>`;}).join('')||'<tr><td colspan="7" style="text-align:center;color:#475569;padding:24px;">لا توجد عروض أسعار</td></tr>'}
      </tbody>
    </table>
  </div>`;
}

window.applyQtFilter = function(){
  const status = document.getElementById('qt-status-filter')?.value||'';
  const list = status ? quotes.filter(q=>q.status===status) : quotes;
  const el = document.getElementById('qt-list');
  if(el) el.innerHTML = qtListHTML(list);
};

window.openQuoteForm = function(id){
  const q = id ? quotes.find(x=>x.id===id)||{} : {};
  _qtSelectedProducts = {}; _qtAddMode = 'product';
  MS = {type:'quoteform', data: JSON.parse(JSON.stringify(q))};
  render();
};
window.deleteQuote = async function(id){
  if(!confirm('حذف عرض السعر؟')) return;
  try{ await api('DELETE','/api/quotes/'+id); await loadAll(); page='quotes'; render(); }
  catch(e){ alert('خطأ: '+e.message); }
};
window.openQuoteDetail = function(id){
  const q = quotes.find(x=>x.id===id);
  if(!q){ alert('غير موجود'); return; }
  MS = {type:'quotedetail', data: q};
  render();
};

function quoteDetailModal(q){
  const nm = q.customer_id ? (q.linked_customer_name||'—') : (q.customer_name||'—');
  const ph = q.customer_id ? (q.linked_customer_phone||'') : (q.customer_phone||'');
  const converted = q.converted_sale_id || q.converted_service_id;
  return `<div class="overlay" id="mover"><div class="modal" style="max-width:640px;">
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
    <div>
      <div style="font-size:16px;font-weight:800;color:#f1f5f9;">عرض سعر #${q.id}</div>
      <div style="font-size:12px;color:#64748b;margin-top:2px;">${q.date} ${q.valid_until?'— صالح حتى '+q.valid_until:''}</div>
    </div>
    <button class="btn s" style="padding:4px 9px;" id="mc">✕</button>
  </div>
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px;">
    <div class="stat"><div style="font-size:11px;color:#64748b;">الزبون</div><div style="font-weight:700;color:#f1f5f9;margin-top:4px;">${esc(nm)}${!q.customer_id?' <span class="badge y" style="font-size:10px;">غير مسجّل</span>':''}</div>${ph?`<div style="font-size:11px;color:#94a3b8;margin-top:2px;">${esc(ph)}</div>`:''}</div>
    <div class="stat"><div style="font-size:11px;color:#64748b;">الحالة</div><div style="margin-top:4px;"><span class="badge ${QUOTE_STATUS_CLS[q.status]||'b'}">${q.status}</span></div></div>
  </div>
  <table><thead><tr>
    <th style="font-size:12px;">النوع</th><th style="font-size:12px;">الوصف</th><th style="font-size:12px;">الكمية</th><th style="font-size:12px;">السعر</th><th style="font-size:12px;">الإجمالي</th>
  </tr></thead><tbody>
  ${(q.items||[]).map(i=>`<tr>
    <td><span class="badge ${i.kind==='service'?'y':'b'}">${i.kind==='service'?'🔧 خدمة':'📦 منتج'}</span></td>
    <td style="font-weight:600;color:#f1f5f9;">${esc(i.kind==='product'?(i.product_name||i.description):i.description)}</td>
    <td style="color:#94a3b8;">${i.qty}</td>
    <td>${(i.price||0).toLocaleString()} ${cur()}</td>
    <td style="color:#52b788;font-weight:700;">${(i.qty*i.price).toLocaleString()} ${cur()}</td>
  </tr>`).join('')}
  </tbody></table>
  <div style="text-align:left;padding:10px 0;font-size:16px;font-weight:800;color:#52b788;">
    الإجمالي: ${(q.total||0).toLocaleString()} ${cur()}
  </div>
  ${q.notes?`<div style="color:#64748b;font-size:12px;margin-bottom:10px;">📝 ${esc(q.notes)}</div>`:''}
  ${converted?`<div style="background:rgba(45,106,79,.15);border:1px solid rgba(45,106,79,.3);border-radius:8px;padding:10px;margin-bottom:14px;font-size:13px;color:#52b788;">
    ✅ تم تحويل هذا العرض ${q.converted_sale_id?'لفاتورة بيع #'+q.converted_sale_id+' ':''}${q.converted_service_id?'ولطلب خدمة #'+q.converted_service_id:''}
  </div>`:''}
  <div style="display:flex;gap:10px;">
    <button class="btn p" onclick="doPrintQuote(${q.id})">🖨️ طباعة</button>
    ${!converted?`<button class="btn p" style="background:linear-gradient(135deg,#1e40af,#1e3a8a);" onclick="convertQuote(${q.id})">✅ تحويل لفاتورة/خدمة</button>`:''}
    ${!converted?`<select class="inp" style="width:140px;" onchange="updateQuoteStatus(${q.id},this.value)">
...
window.updateQuoteStatus = async function(id, status){
  if(!status) return;
  try{ await api('POST','/api/quotes/'+id+'/status', {status}); await loadAll(); openQuoteDetail(id); }
  catch(e){ alert('خطأ: '+e.message); }
};
...
window.doConvertQuote = async function(id, serialsMap){
  ...
  try{
    ...
    if(res.service_id) msg += ` — طلب خدمة #${res.service_id}`;
    alert(msg);
    closeM();
  }catch(e){ alert('خطأ: '+e.message); }
};

window.doPrintQuote = function(id){
  const q = quotes.find(x=>x.id===id);
  if(!q) return;
  const nm = q.customer_id ? (q.linked_customer_name||'—') : (q.customer_name||'—');
  const html = `<!DOCTYPE html><html lang="ar" dir="rtl"><head><meta charset="UTF-8">
  <link href="https://fonts.googleapis.com/css2?family=Tajawal:wght@400;700;800&display=swap" rel="stylesheet">
  <style>*{font-family:'Tajawal',sans-serif;box-sizing:border-box}body{background:#fff;color:#111;padding:24px;direction:rtl}
  table{width:100%;border-collapse:collapse;margin:14px 0}
  th{background:#d4edda;font-weight:700;padding:8px 10px;text-align:right;border:1px solid #aaa;font-size:13px}
  td{padding:7px 9px;border:1px solid #ddd;font-size:13px}
  h1{font-size:20px;margin-bottom:4px} .sub{color:#555;font-size:13px;margin-bottom:16px}
  @page{size:A4;margin:14mm}</style></head><body onload="window.print()">
  <h1>عرض سعر #${q.id} — ${esc(sysSettings.system_name||'')}</h1>
  <div class="sub">التاريخ: ${q.date} ${q.valid_until?'— صالح حتى: '+q.valid_until:''}</div>
  <div class="sub">الزبون: ${esc(nm)} ${(q.customer_phone||q.linked_customer_phone)?' — '+esc(q.customer_phone||q.linked_customer_phone):''}</div>
  <table><thead><tr><th>الوصف</th><th>الكمية</th><th>السعر</th><th>الإجمالي</th></tr></thead><tbody>
  ${(q.items||[]).map(i=>`<tr><td>${esc(i.kind==='product'?(i.product_name||i.description):i.description)}</td><td>${i.qty}</td><td>${(i.price||0).toLocaleString()} ${cur()}</td><td>${(i.qty*i.price).toLocaleString()} ${cur()}</td></tr>`).join('')}
  </tbody></table>
  <div style="text-align:left;font-size:17px;font-weight:800;">الإجمالي: ${(q.total||0).toLocaleString()} ${cur()}</div>
  ${q.notes?`<div style="margin-top:14px;color:#555;">ملاحظات: ${esc(q.notes)}</div>`:''}
  </body></html>`;
...
window.accTab = async function(tab){
  ...
  if(tab==='sum'){
    el.innerHTML = accSumContentHTML();
    return;
  }
...
function paySummaryHTML(data, name, party_type, party_id){
  ...
  + '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:16px;">'
    ...
    + '</div>'
    + '<div class="stat" style="text-align:center;"><div style="font-size:12px;color:#64748b;">المتبقي</div><div style="font-size:18px;font-weight:800;color:'+(total_remaining>0?'#f87171':'#52b788')+';margin-top:6px;">'+total_remaining.toLocaleString()+' '+cur()+'</div>'
      + (data.opening_balance?('<div style="font-size:10px;color:#94a3b8;margin-top:2px;">شامل '+data.opening_balance.toLocaleString()+' '+cur()+' رصيد افتتاحي</div>'):'')
    + '</div>'
  + '</div>'
  + '<div class="card"><table>'
...
function entModal(item){
  ...
  <div class="g2">
    ${[['name','الاسم *'],['contact','جهة الاتصال'],['phone','الهاتف'],
       ['email','البريد'],['city','المدينة']].map(([k,l])=>`
    <div><label class="lbl">${l}</label>
      <input class="inp" id="ef${k}" type="text" value="${item[k]??''}"/>
    </div>`).join('')}
    <div style="grid-column:span 2">
      <label class="lbl">الرصيد الافتتاحي</label>
      <input class="inp" id="efbalance" type="number" step="0.01" value="${item.balance??0}"/>
      <div style="font-size:11px;color:#64748b;margin-top:4px;">
        مبلغ سابق قبل استخدام النظام — قيمة موجبة تُضاف للمستحق الحالي، وقيمة سالبة تُعتبر رصيداً دائناً يُخصم من المستحق. اتركه 0 إن لم يوجد رصيد سابق.
      </div>
    </div>
...
// ── مودال نموذج عرض السعر (إنشاء/تعديل) ──
let _qtItems = [];

// اختيار متعدد للمنتجات بعرض السعر عبر تشيك بوكس: product_id -> {checked, qty, price}
let _qtSelectedProducts = {};

function qtProductChecklistHTML(query){
  const q = (query||'').trim().toLowerCase();
  const list = products.filter(p=>!q || p.name.toLowerCase().includes(q) || (p.barcode||'').toLowerCase().includes(q));
  if(!list.length) return '<div style="text-align:center;color:#64748b;padding:14px;font-size:12px;">لا توجد منتجات مطابقة</div>';
  return list.slice(0,150).map(p=>{
    const sel = _qtSelectedProducts[p.id] || {};
    const qty = sel.qty || 1;
    const price = sel.price != null ? sel.price : p.sell_price;
    return `<div style="display:flex;align-items:center;gap:8px;padding:6px 4px;border-bottom:1px solid #1e2537;">
      <input type="checkbox" ${sel.checked?'checked':''} onchange="qtToggleProduct(${p.id})" style="width:16px;height:16px;accent-color:#2d6a4f;flex-shrink:0;"/>
      <span style="flex:1;font-size:12px;color:#f1f5f9;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${esc(p.name)}">${esc(p.name)} <span style="color:#64748b;font-family:monospace;font-size:10px;">${p.barcode}</span></span>
      <input type="number" min="1" value="${qty}" style="width:55px;padding:4px 6px;font-size:12px;" class="inp" onchange="qtSetProductQty(${p.id},this.value)"/>
      <input type="number" min="0" step="0.01" value="${price}" style="width:75px;padding:4px 6px;font-size:12px;" class="inp" onchange="qtSetProductPrice(${p.id},this.value)"/>
    </div>`;
  }).join('');
}

window.qtFilterProducts = function(){
  const q = document.getElementById('qtf-prod-search')?.value||'';
  const el = document.getElementById('qtf-prod-list');
  if(el) el.innerHTML = qtProductChecklistHTML(q);
};

window.qtToggleProduct = function(pid){
  const p = products.find(x=>x.id===pid);
  if(!p) return;
  if(!_qtSelectedProducts[pid]) _qtSelectedProducts[pid] = {qty:1, price:p.sell_price, checked:false};
  _qtSelectedProducts[pid].checked = !_qtSelectedProducts[pid].checked;
};
window.qtSetProductQty = function(pid, val){
  if(!_qtSelectedProducts[pid]) _qtSelectedProducts[pid] = {checked:true, price:products.find(x=>x.id===pid)?.sell_price||0};
  _qtSelectedProducts[pid].qty = Math.max(1, parseInt(val)||1);
};
window.qtSetProductPrice = function(pid, val){
  if(!_qtSelectedProducts[pid]) _qtSelectedProducts[pid] = {checked:true, qty:1};
  _qtSelectedProducts[pid].price = parseFloat(val)||0;
};

function quoteFormModal(item){
  const isEdit = !!item.id;
  const today = new Date().toISOString().slice(0,10);
  if(!MS._qtItemsInit){
    _qtItems = (item.items||[]).map(i=>({kind:i.kind, product_id:i.product_id, description:i.description,
                                          name:i.kind==='product'?(i.product_name||i.description):i.description,
                                          qty:i.qty, price:i.price}));
    MS._qtItemsInit = true;
  }
  const total = _qtItems.reduce((s,i)=>s+i.qty*i.price,0);
  const usingExisting = !!(item.customer_id || (!item.customer_name && !isEdit));
  return `<div class="overlay" id="mover"><div class="modal" style="max-width:680px;">
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
    <div style="font-size:16px;font-weight:700;color:#f1f5f9;">${isEdit?'✏️ تعديل عرض سعر #'+item.id:'📄 عرض سعر جديد'}</div>
    <button class="btn s" style="padding:4px 9px;" id="mc">✕</button>
  </div>
  <div id="merr"></div>

  <div style="display:flex;gap:8px;margin-bottom:10px;">
    <button type="button" class="btn ${usingExisting?'p':'s'}" id="qtf-mode-existing" style="flex:1;justify-content:center;">👤 زبون مسجّل</button>
    <button type="button" class="btn ${!usingExisting?'p':'s'}" id="qtf-mode-new" style="flex:1;justify-content:center;">➕ زبون غير مسجّل بعد</button>
  </div>
  <div id="qtf-existing-wrap" style="display:${usingExisting?'block':'none'};margin-bottom:12px;">
    <label class="lbl">اختر الزبون</label>
    <select class="inp" id="qtf-customer">
      <option value="">-- اختر --</option>
      ${customers.map(c=>`<option value="${c.id}"${item.customer_id===c.id?' selected':''}>${esc(c.name)}</option>`).join('')}
    </select>
  </div>
  <div id="qtf-new-wrap" style="display:${!usingExisting?'block':'none'};margin-bottom:12px;">
    <div class="g2">
      <div><label class="lbl">اسم الزبون</label><input class="inp" id="qtf-cname" value="${esc(item.customer_name)||''}"/></div>
      <div><label class="lbl">رقم الهاتف (اختياري)</label><input class="inp" id="qtf-cphone" value="${esc(item.customer_phone)||''}"/></div>
    </div>
    <div style="font-size:11px;color:#64748b;margin-top:4px;">💡 لو العرض اتقبل، رح يتسجّل كزبون جديد تلقائياً بالنظام</div>
  </div>

  <div class="g2" style="margin-bottom:12px;">
    <div><label class="lbl">التاريخ</label><input class="inp" type="date" id="qtf-date" value="${item.date||today}"/></div>
    <div><label class="lbl">صالح حتى (اختياري)</label><input class="inp" type="date" id="qtf-valid" value="${item.valid_until||''}"/></div>
    <div style="grid-column:span 2;"><label class="lbl">ملاحظات</label><input class="inp" id="qtf-notes" value="${esc(item.notes)||''}"/></div>
  </div>

  <div style="background:#1a1d27;border:1px solid #2d3349;border-radius:8px;padding:12px;margin-bottom:12px;">
    <div style="font-size:13px;font-weight:700;color:#52b788;margin-bottom:10px;">➕ إضافة صنف</div>
    <div style="display:flex;gap:6px;margin-bottom:8px;">
      <button type="button" class="btn ${_qtAddMode!=='service'?'p':'s'}" id="qtf-item-mode-prod" style="flex:1;justify-content:center;font-size:12px;">📦 منتجات من المخزون</button>
      <button type="button" class="btn ${_qtAddMode==='service'?'p':'s'}" id="qtf-item-mode-svc" style="flex:1;justify-content:center;font-size:12px;">🔧 خدمة (وصف حر)</button>
    </div>
    <div id="qtf-item-fields">
      ${_qtAddMode==='service' ? `
      <div style="display:flex;gap:8px;flex-wrap:wrap;">
        <input class="inp" id="qtf-item-desc" placeholder="وصف الخدمة (مثال: تركيب نظام كاميرات)" style="flex:2;min-width:200px;"/>
        <input class="inp" type="number" id="qtf-item-qty" placeholder="الكمية" value="1" min="1" style="width:80px;"/>
        <input class="inp" type="number" id="qtf-item-price" placeholder="السعر" style="width:100px;"/>
        <button type="button" class="btn p" id="qtf-item-add">+ إضافة</button>
      </div>
      ` : `
      <input class="inp" id="qtf-prod-search" placeholder="🔍 بحث بالاسم أو الباركود..." style="margin-bottom:8px;width:100%;"/>
      <div id="qtf-prod-list" style="max-height:220px;overflow-y:auto;background:#161923;border:1px solid #2d3349;border-radius:8px;padding:6px;">
        ${qtProductChecklistHTML('')}
      </div>
      <button type="button" class="btn p" id="qtf-item-add" style="width:100%;justify-content:center;margin-top:8px;">✅ إضافة المنتجات المحدَّدة</button>
      `}
    </div>
  </div>
  ...
  <div class="card" style="margin-bottom:14px;">
    <table>
      <thead><tr><th>النوع</th><th>الوصف</th><th>الكمية</th><th>السعر</th><th>الإجمالي</th><th></th></tr></thead>
      <tbody id="qtf-items-body">
      ${_qtItems.map((i,idx)=>`<tr>
        <td><span class="badge ${i.kind==='service'?'y':'b'}">${i.kind==='service'?'🔧 خدمة':'📦 منتج'}</span></td>
        <td style="font-weight:600;color:#f1f5f9;">${esc(i.name)}</td>
        <td>${i.qty}</td>
        <td>${i.price.toLocaleString()} ${cur()}</td>
        <td style="color:#52b788;font-weight:700;">${(i.qty*i.price).toLocaleString()} ${cur()}</td>
        <td><button type="button" class="btn d" style="padding:3px 7px;" onclick="removeQtItem(${idx})">🗑️</button></td>
      </tr>`).join('') || '<tr><td colspan="6" style="text-align:center;color:#64748b;padding:10px;">لا توجد أصناف</td></tr>'}
      </tbody>
    </table>
    <div style="text-align:left;padding:8px 14px;font-size:14px;font-weight:800;color:#52b788;border-top:1px solid #1e2537;">
      الإجمالي: ${total.toLocaleString()} ${cur()}
    </div>
  </div>

  <div style="display:flex;gap:10px;">
    <button class="btn p" id="ms" style="flex:1;justify-content:center;">💾 ${isEdit?'حفظ التعديلات':'إنشاء عرض السعر'}</button>
    <button class="btn s" id="mc2">إلغاء</button>
  </div>
  </div></div>`;
}

let _qtAddMode = 'product';

// حفظ قيم حقول نموذج عرض السعر الحالية بالذاكرة قبل أي إعادة رسم، حتى لا تُفقد (اسم الزبون، التاريخ...)
function saveQuoteFormDraft(){
  if(!MS || MS.type!=='quoteform') return;
  const d = MS.data;
  const usingExisting = document.getElementById('qtf-existing-wrap')?.style.display !== 'none';
  if(usingExisting){
    const cid = document.getElementById('qtf-customer')?.value;
    d.customer_id = cid ? parseInt(cid) : null;
    d.customer_name = '';
    d.customer_phone = '';
  } else {
    d.customer_id = null;
    d.customer_name = document.getElementById('qtf-cname')?.value || '';
    d.customer_phone = document.getElementById('qtf-cphone')?.value || '';
  }
  d.date = document.getElementById('qtf-date')?.value || d.date;
  d.valid_until = document.getElementById('qtf-valid')?.value || '';
  d.notes = document.getElementById('qtf-notes')?.value || '';
}

window.removeQtItem = function(idx){
  saveQuoteFormDraft();
  _qtItems.splice(idx,1);
  render();
};
...
function bindModal(){
  ...
  // ── نموذج عرض السعر ──
  if(MS?.type==='quoteform'){
    document.getElementById('qtf-mode-existing')?.addEventListener('click', ()=>{
      document.getElementById('qtf-existing-wrap').style.display='block';
      document.getElementById('qtf-new-wrap').style.display='none';
      document.getElementById('qtf-mode-existing').className='btn p';
      document.getElementById('qtf-mode-new').className='btn s';
    });
    document.getElementById('qtf-mode-new')?.addEventListener('click', ()=>{
      document.getElementById('qtf-existing-wrap').style.display='none';
      document.getElementById('qtf-new-wrap').style.display='block';
      document.getElementById('qtf-mode-new').className='btn p';
      document.getElementById('qtf-mode-existing').className='btn s';
    });
    document.getElementById('qtf-item-mode-prod')?.addEventListener('click', ()=>{ saveQuoteFormDraft(); _qtAddMode='product'; render(); });
    document.getElementById('qtf-item-mode-svc')?.addEventListener('click', ()=>{ saveQuoteFormDraft(); _qtAddMode='service'; render(); });
    document.getElementById('qtf-prod-search')?.addEventListener('input', qtFilterProducts);
    document.getElementById('qtf-item-add')?.addEventListener('click', ()=>{
      if(_qtAddMode==='service'){
        const qty = parseInt(document.getElementById('qtf-item-qty')?.value)||1;
        const price = parseFloat(document.getElementById('qtf-item-price')?.value)||0;
        const desc = document.getElementById('qtf-item-desc')?.value?.trim();
        if(!desc){ alert('اكتب وصف الخدمة'); return; }
        _qtItems.push({kind:'service', product_id:null, description:desc, name:desc, qty, price});
      } else {
        const selected = Object.entries(_qtSelectedProducts).filter(([,v])=>v.checked);
        if(!selected.length){ alert('حدّد منتج واحد على الأقل عبر التشيك بوكس'); return; }
        selected.forEach(([pid,v])=>{
          const p = products.find(x=>x.id===parseInt(pid));
          if(!p) return;
          _qtItems.push({kind:'product', product_id:p.id, description:p.name, name:p.name,
                         qty: v.qty||1, price: v.price!=null?v.price:p.sell_price});
        });
        _qtSelectedProducts = {};
      }
      saveQuoteFormDraft();
    ...
    document.getElementById('ms').onclick = async()=>{
      ...
      if(!date){ errEl.innerHTML='<div class="err">يرجى تحديد التاريخ</div>'; return; }
      if(!_qtItems.length){ errEl.innerHTML='<div class="err">أضف صنفاً واحداً على الأقل</div>'; return; }
      const body = {
        date, valid_until: document.getElementById('qtf-valid')?.value||'',
        notes: document.getElementById('qtf-notes')?.value||'',
        status: MS.data?.status || 'مسودة',
        items: _qtItems.map(i=>({kind:i.kind, product_id:i.product_id, description:i.description, qty:i.qty, price:i.price})),
      };
      if(usingExisting){
        const cid = document.getElementById('qtf-customer')?.value;
        if(!cid){ errEl.innerHTML='<div class="err">يرجى اختيار الزبون</div>'; return; }
        body.customer_id = parseInt(cid);
      } else {
        const cname = document.getElementById('qtf-cname')?.value?.trim();
        if(!cname){ errEl.innerHTML='<div class="err">يرجى كتابة اسم الزبون</div>'; return; }
        body.customer_name = cname;
        body.customer_phone = document.getElementById('qtf-cphone')?.value||'';
      }
      try{
        if(MS.data?.id) await api('PUT','/api/quotes/'+MS.data.id, body);
...
function bindPage(){
  ...
  // Quotes
  if(page==='quotes'){
    document.getElementById('qt-add-btn')?.addEventListener('click', ()=>{ _qtItems=[]; _qtSelectedProducts={}; _qtAddMode='product'; MS={type:'quoteform', data:{}}; render(); });
    document.getElementById('qt-status-filter')?.addEventListener('change', applyQtFilter);
  }
